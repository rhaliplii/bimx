...
for f in sorted((CONTENT / "ro").glob("*.json")) + sorted((CONTENT / "en").glob("*.json")):
    ...
    for c in d["courses"]:
        for l in c["lessons"]:
            ...
            if f.parent.name != "en" and re.search(r"[şţŞŢ]", " ".join(text)): errors.append(f"{where}: diacritice cu sedilă (ş/ţ) în loc de virgulă")
...
# Traducerea EN a paginilor bimx.md: fiecare text al paginilor RO are traducere, cu aceleași marcaje {n}.
sys.path.insert(0, str(Path(__file__).resolve().parent))
from sitegen import translate  # noqa: E402

MIRROR = Path(__file__).resolve().parent.parent / "src" / "bimx-mirror"
catalog = json.loads((MIRROR / "i18n" / "en.json").read_text(encoding="utf-8"))["text"]
needed = {}
...
for key, page in needed.items():
    if key not in catalog:
        errors.append(f"i18n/en.json: lipsește traducerea pentru „{key[:80]}” ({page})")
    elif not translate.check_placeholders(key, catalog[key]):
        errors.append(f"i18n/en.json: marcajele {{n}} diferă în „{key[:80]}”")
unused = len(set(catalog) - set(needed))
print(f"bimx.md EN: {len(needed)} texte, {len(needed) - sum(k not in catalog for k in needed)} traduse"
